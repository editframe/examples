...
// CSS cubic-bezier equivalents of the power-curve/spring easings this project used to
// compute per-frame. `out-back` is the standard "ease-out-back" overshoot curve — the
// closest CSS analogue of the hand-rolled `outBack()` spring helper used throughout the
// original onFrame code (FN's brand voice leans on that punchy overshoot a lot).
const EASE_CSS: Record<Easing, string> = {
  ...
  "out-back": "cubic-bezier(0.34,1.56,0.64,1)", // matches outBack(~1.7) overshoot
};
...
export type RevealProps = {
  ...
  /** px to float up from on enter (default 20; pass 0 for a plain fade). */
  y?: number;
  /**
   * Exit fade. Either:
   ...
   *   the scene boundary.
   * Omit entirely for elements with no exit (e.g. a closing beat that just holds).
   */
