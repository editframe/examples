/**
 * HeroToOverview — Merged Scene 1 + 2 + 3
...
import { track, lerp } from "@shared/utils/animation";
import { resolveFrame } from "@shared/camera/resolveFrame";
import { dollyIn, dollyOut, tilt } from "@shared/camera/shots";
import { frameToTransform } from "@shared/camera/types";
...
// Former flat `scale()` magnitudes (1.4 close-up / 0.9 wide / 1.35 filter-punch) — now
// expressed as z-depth dollies in `CAMERA_STOPS` below (see `dollyIn`/`dollyOut` calls).
// Kept here only as the historical basis for the FILTER_TX/FILTER_TY derivation in the
// comment block below, which pre-dates the `<Camera>`/`resolveFrame` migration.
...
/**
 * Camera shot list — a `useCameraFrame` retrofit of the hand-rolled `camScale`/`camTx`/
 * `camTy` if/else ladder that used to live directly in `onFrame` below. Kept as a call
 * inside this scene's own `onFrame` (rather than wrapping content in `<Camera>`) because
 * the camera, scroll, cursor pathing, and dropdown state all read the same `ms` clock in
 * one procedural unit (see the file-level doc comment) — `useCameraFrame` shares the exact
 * same `resolveFrame` core as `<Camera>` without forcing a second nested `Timegroup`.
 *
 * The two zoom-ins are now real `dollyIn` + `tilt` moves (z-depth + a couple degrees of
 * rotation) instead of a flat `scale()`, so they read as camera dolly moves with genuine
 * perspective foreshortening under `<Camera>`'s default `perspective` rather than a CSS zoom.
 */
const CAMERA_STOPS: CameraStop[] = [
  { at: 0, frame: tilt(dollyIn(400, { x: 0, y: 286 }), 2, -3) },
  { at: ZOOM_OUT_START, frame: tilt(dollyIn(400, { x: 0, y: 286 }), 2, -3), hold: true },
  { at: ZOOM_OUT_END, frame: dollyOut(-156), ease: "inOutCubic" },
  { at: ZOOM_IN2_START, frame: dollyOut(-156), hold: true },
  { at: ZOOM_IN2_END, frame: tilt(dollyIn(363, { x: FILTER_TX, y: FILTER_TY }), -2, 4), ease: "inOutCubic" },
  { at: FILTER_APPLIED, frame: tilt(dollyIn(363, { x: FILTER_TX, y: FILTER_TY }), -2, 4), hold: true },
  { at: REVEAL_END, frame: dollyOut(-156), ease: "inOutCubic" },
];
...
export function HeroToOverview() {
  ...
  const onFrame = useCallback(({ ownCurrentTimeMs: ms }: { ownCurrentTimeMs: number }) => {
    // ── CAMERA — one `resolveFrame` call instead of a hand-rolled scale/tx/ty if/else
    // ladder (see `CAMERA_STOPS` above). `resolveFrame` is the same evaluator `<Camera>`
    // uses; called directly (not via the `useCameraFrame` hook wrapper) since this runs
    // inside an imperative per-frame callback, not a render — see `useCameraFrame.ts`'s
    // doc comment for when to reach for the hook form instead. ──
    if (cameraRef.current) {
      const frame = resolveFrame(CAMERA_STOPS, ms);
      cameraRef.current.style.transform = frameToTransform(frame);
    }
  ...
  return (
    ...
    >
      ...
      <Audio src="/vercel-knowledge-base-demo/src/assets/sfx/vercel-knowledge-base-demo-click.mp3" offset={`${CLICK_BTN_TIME}ms`} sourceIn="0.6s" duration="0.3s" volume={1} />
      {/* Sandbox row click — closes the dropdown, applies the filter */}
      <Audio src="/vercel-knowledge-base-demo/src/assets/sfx/vercel-knowledge-base-demo-click.mp3" offset={`${SANDBOX_CLICK}ms`} sourceIn="0.6s" duration="0.3s" volume={1} />
    </Timegroup>
  );
}
