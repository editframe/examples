...
type Easing = "out-cubic" | "in-cubic" | "in-out-quad" | "out-expo";
...
const EASE_CSS: Record<Easing, string> = {
  ...
  "in-out-quad": "cubic-bezier(0.45,0,0.55,1)", // matches easeInOutQuad / easeInOutCubic
  "out-expo": "cubic-bezier(0.16,1,0.3,1)",     // matches easeOutExpo
};
...
export type RevealProps = {
  ...
  /** px to float up from on enter (default 20, use 0 to disable the float and just cross-fade). */
  y?: number;
  /**
   * Exit fade. Either:
   ...
   *   the scene boundary (e.g. a hard cut to the next item in a cycling list).
 ...
 * Use this for one-shot "float up + fade in, later fade out" callouts, and (with `y={0}`
 * and a short `[delay, end]` window) for discrete hard-cut swaps like a cycling selection.
 */
